...
class Model():
	def predict(self, X):
		return self.full_pipeline_for_installs.predict(X)
	def install_count_string_to_numeric(self, obj):
		result = ''.join(obj.split('+')[0].split(','))
		return result
	def __init__(self, X, y=None):
		self.X = X
		self.y = y.apply(self.install_count_string_to_numeric).copy()
		self.categorical_features = ['Category', 'Content Rating', 'Android Ver']
		self.numerical_features = ['Size', 'Price', 'Reviews', 'Rating']
